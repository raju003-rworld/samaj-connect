...
import { Heart, MessageCircle, Share2, Bookmark, Trash2, Flag, MoreHorizontal, ChevronLeft, ChevronRight, Eye, MapPin, Calendar, Repeat2, ExternalLink } from "lucide-react";
...
export const Media = ({ urls }) => {
  const [i, setI] = useState(0);
  if (!urls?.length) return null;
  const u = urls[i];
  return (
    <div className="relative mt-3 rounded-2xl overflow-hidden border border-slate-100 bg-black">
      {isVideoUrl(u) ? <video src={u} controls playsInline preload="metadata" className="w-full max-h-[70vh] object-contain" data-testid="post-video" />
        : <img src={u} alt="" loading="lazy" className="w-full max-h-[70vh] object-contain" />}
      {urls.length > 1 && (
        <>
...
export function PostCard({ p, onChange, onRemove, onCreated, autoOpenComments = false }) {
  ...
  const del = async () => { if (!window.confirm("પોસ્ટ ડિલીટ કરવી?")) return; await api.delete(`/posts/${p.id}`); onRemove(p.id); };
  const report = async () => {
    ...
    <article ref={ref} data-testid={`post-card-${p.id}`} className="bg-white rounded-3xl p-4 border border-slate-100 shadow-sm">
      <header className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {menu && (
            <div className="absolute right-0 mt-1 bg-white border border-slate-200 rounded-2xl shadow-lg py-1 w-44 z-10 text-sm">
      ...
      {p.sharedFrom ? <SharedFrom s={p.sharedFrom} /> : <Media urls={p.mediaUrls} />}
